...
static void loadTexture(GLuint texHandle, const char *ppmFilename) {
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); // changed from GL_CLAMP to GL_CLAMP_TO_EDGE to fix runtime error
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE); // changed from GL_CLAMP to GL_CLAMP_TO_EDGE to fix runtime error
